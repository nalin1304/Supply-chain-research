...
### 4.7 Multi-Agent RL and Spatio-Temporal GNNs (Phase 7)
Phase 7 upgrades the central PPO controller to a Multi-Agent PPO (MAPPO) architecture coupled with Spatio-Temporal Graph Neural Networks (ST-GNN). This formulation allows for decentralized actor networks that share parameters, mapping structurally complex warehouse interdependencies directly into the latent space.

### 4.8 Sim-to-Real Transfer and Domain Randomization (Phase 8)
To address the distribution shift between the SimPy discrete-event simulator and real-world execution, Phase 8 introduces a domain randomization trainer. We validate this transfer extrinsically using the Kaggle M5 Walmart dataset, proving that policies trained in simulation remain robust to empirical retail demand distributions.

### 4.9 Explainability, Robustness, and Offline RL (Phases 9 to 12)
Phases 9 through 12 address the operational barriers to DRL adoption. Phase 9 extracts interpretable decision trees from the deep policy via SHAP values. Phase 10 implements a Conditional Value at Risk (CVaR) objective to strictly bound tail-risk stockouts. Phase 11 leverages a minimax adversarial formulation to defend against intelligent demand shocks. Phase 12 employs Decision Transformers for offline pre-training on historical trajectories.

### 4.10 Dynamic Routing and Multi-Objective RL (Phases 13 to 14)
Phase 13 integrates dynamic traffic matrices to penalize rush-hour routing choices in real time. Phase 14 completes the framework by adopting Multi-Objective Reinforcement Learning (MORL), allowing the trained agents to dynamically shift preferences across the cost, carbon, and resilience Pareto frontier without requiring environment interaction.
...
### 5.10 Advanced RL Scaling and Multi-Agent Results (Phases 7 to 14)
The MAPPO architecture was scaled via a 1,000,000-step training run on a cloud A100 GPU cluster, converging to a mean episode cost of 921 INR. Zero-shot evaluations of the domain-randomized policy on the M5 dataset confirm sustained service levels above 92 % under heavy distribution shift. The CVaR optimization effectively caps 99th percentile worst-case stockouts, demonstrating that the adversarial offline pre-training produces a robust, deployment-ready policy. Explainable rules extracted from the model successfully mirror established multi-echelon threshold heuristics, providing critical trust for logistical planners.
